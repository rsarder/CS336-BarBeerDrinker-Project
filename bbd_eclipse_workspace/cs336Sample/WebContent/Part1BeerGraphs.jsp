...
<% 
	...
	try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		//Make a query
   		String query = "" ;
   		query = "SELECT bills.day as days, sum(quantity) as beers_sold FROM transactions "+
   				"LEFT JOIN bills ON transactions.bill_id = bills.bill_id "+
   				"WHERE transactions.item = \'" + typeName + "\'	GROUP BY day " +
   		...
   		while (result.next()) { 
   			map=new HashMap<String,Integer>();
   	   		map.put(result.getString("days").substring(0,result.getString("days").length()-1),result.getInt("beers_sold"));
   			list.add(map);
   	    } 
   	    result.close();
        ...
        //Create the chart title according to what the user selected
   		chartTitle = "Beers Sold Over a Week";
        legend = "Day";
